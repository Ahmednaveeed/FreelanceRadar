...
export default function AuthPage() {
  ...
  const inputStyle = {
    ...
    color: '#fff', outline: 'none', transition: 'border 0.2s ease',
    fontFamily: 'var(--font-inter)'
  }

  return (
    <main style={{
      minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 24,
      background: 'radial-gradient(ellipse at top, rgba(16,185,129,0.08) 0%, transparent 60%), #0a0a0a'
    }}>
      ...
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1, duration: 0.5 }}
        ...
        <div style={{ display: 'flex', background: 'rgba(255,255,255,0.04)', borderRadius: 12, padding: 4, marginBottom: 32 }}>
          {['Sign Up', 'Log In'].map((tab, i) => (
            <button key={tab} onClick={() => { setIsLogin(i === 1); setError(''); setMessage('') }}
              style={{
                flex: 1, padding: '10px 0', borderRadius: 9, fontSize: 14, fontWeight: 600,
                border: 'none', cursor: 'pointer', transition: 'all 0.2s ease',
                background: isLogin === (i === 1) ? 'linear-gradient(135deg, #10b981, #059669)' : 'transparent',
                color: isLogin === (i === 1) ? '#fff' : '#6b7280',
                fontFamily: 'var(--font-inter)'
              }}>
              {tab}
            </button>
        ...
        <AnimatePresence mode="wait">
          <motion.div key={isLogin ? 'login' : 'signup'}
            ...
            <h2 style={{ fontFamily: 'var(--font-syne)', fontSize: 24, fontWeight: 800, marginBottom: 6 }}>
              {isLogin ? 'Welcome back' : 'Create your account'}
            </h2>
            <p style={{ color: '#6b7280', fontSize: 14, marginBottom: 28 }}>
              {isLogin ? 'Log in to see your job matches.' : 'Start receiving AI-matched jobs in minutes.'}
            </p>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              {message && (
                <div style={{ background: 'rgba(16,185,129,0.1)', border: '1px solid rgba(16,185,129,0.2)', borderRadius: 8, padding: '10px 14px', fontSize: 13, color: '#34d399' }}>
                  {message}
                </div>
              )}
              ...
              <button type="submit" className="btn-primary" disabled={loading}
                style={{
                  ...
                  marginTop: 4, opacity: loading ? 0.7 : 1, fontFamily: 'var(--font-inter)'
                }}>
      ...
      <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }}
        style={{ marginTop: 24, fontSize: 12, color: '#374151', textAlign: 'center' }}>
